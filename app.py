...
st.markdown("""
<p style='font-size:medium; color:red;'>Disclaimer: This is not financial advice.</p>
<p style='font-size:Large; color:black;'>(Put summary here please)</p>
<hr style='border: 2px solid black;'>
<p style='font-size:xx-Large; color:black;'>Portfolio Allocation</p>
""", unsafe_allow_html=True)

with st.expander("Click to show more"):
    st.write("This is the additional text that will be displayed when the expander is clicked.")
...
# User Inputs
risk_levels = {"Very Low": 73, "Low": 54, "Medium": 36, "High": 18, "Very High": 1}
selected_risk_level = st.sidebar.selectbox("Select Your Portfolio Risk Tolerance:", options=list(risk_levels.keys()))
...
initial_result = minimize(portfolio_performance, initial_guess, args=(risk_aversion, extended_returns, extended_cov_matrix), method='SLSQP', bounds=[(0, 1)] * len(extended_returns), constraints=constraints)

if initial_result.success:
    # Display the initial optimized portfolio weights
    st.write(f"Optimized Portfolio for Risk Aversion {risk_aversion}:")
    st.write(f"Expected Return: {initial_result.fun:.2%}, Volatility: {np.sqrt(np.dot(initial_result.x.T, np.dot(extended_cov_matrix, initial_result.x))):.2%}")
    st.write("Portfolio Weights:")
    for ticker, weight in zip(df['Ticker'].tolist() + ['Risk-Free Asset'], initial_result.x):
        st.write(f"{ticker}: {weight*100:.2f}%")

# Risk Level Results
risk_level_results = []
for level, ra in risk_levels.items():
    result = minimize(portfolio_performance, initial_guess, args=(ra, extended_returns, extended_cov_matrix), method='SLSQP', bounds=[(0, 1)] * len(extended_returns), constraints=constraints)
    if result.success:
        risk_level_results.append({
            "Risk Level": level,
            "Risk Aversion": ra,
            "Expected Return": np.sum(result.x * extended_returns),
            "Volatility": np.sqrt(np.dot(result.x.T, np.dot(extended_cov_matrix, result.x))),
            "Weights": result.x
        })
...
for res in risk_level_results:
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        name=res["Risk Level"],
        marker=dict(size=12 if res["Risk Level"] == selected_risk_level else 8, symbol="star" if res["Risk Level"] == selected_risk_level else "circle")
    ))
fig.update_layout(title="Risk Aversion Levels: Expected Return vs. Volatility", xaxis_title="Volatility (Standard Deviation)", yaxis_title="Expected Return", legend_title="Risk Levels", hovermode="closest")
